...
class Student():
    ...
    def show(self):
        print(f'我叫:{self.name}, 今年:{self.age}')   # 因为xm和age是局部变量，所以这里要用实例属性，示例属性是可以再整个类中去使用的
    ...
    def sm():
        #print(self.name)
        #self.show()
        print('这是一个静态方法,不能调用实用属性,也不能调用实例方法')
...
# 创建类的对象
stu=Student('ydy',18) # 为什么传了两个参数? 因为__init__方法中,有两个形参,self,是自带的参数,无需手动传入
# 实例属性,使用对象名进行打点调用
